...
param (
    ...
    [string]$ProjectName = "Northwind.DataContext.Sqlite",
...
<#
.SYNOPSIS
    Creates a new .NET class library and optionally adds it to an existing solution.

.DESCRIPTION
    This script creates a new .NET class library with specified parameters and can add it
...
.EXAMPLE
    ...
    pwsh -Command ".\scripts\New-DotNetClassLib.ps1"
    
    .\New-DotNetClassLib.ps1 -ProjectName "MyProject.Core"

.EXAMPLE
    .\New-DotNetClassLib.ps1 -ProjectName "MyProject.Domain" -DotNetVersion "8.0"

    # Run the script with what-if from the solution root
    pwsh -Command ".\scripts\New-DotNetClassLib.ps1 -WhatIf"
    pwsh -Command ".\scripts\New-DotNetClassLib.ps1 -ProjectName 'Test.Core' -WhatIf"
...
function New-Project {
    ...
    if ($PSCmdlet.ShouldProcess($cmdString, "Create new project")) {
        Write-Host "Creating new class library: $ProjectName"
        & dotnet $projectArgs
